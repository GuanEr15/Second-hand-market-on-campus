...
//注册
	...
	// 		type: "POST" ,
	// 		url: "check_username",
	// 		data: "username=" + $("#username").val() + '&flag=' + 1,
	// 		dataType:'json',
	// 		success: function(res) {
	// 			if(res.data == false){
	// 				$('#user-sn').css({
	...
	// 					boxShadow: "0 0 2px red"
	// 				});
	// 				$("#userCue").html("<font color='red'><b>昵称已存在</b></font>");
	// 				return false;
	// 			}else {
	// 				$('#user-sn').css({
	// 					border: "1px solid #D7D7D7",
	...
	//
    //     var mailZE = /^[a-zA-Z0-9_-]+@[a-zA-Z0-9_-]+(\.[a-zA-Z0-9_-]+)+$/;
    //     if ($('#email').val()!='' && $('#email').val()!= null && $('#email').val() != mailZE) {
    //         $('#email').css({
    //             border: "1px solid #D7D7D7",
    //             boxShadow: "none"
    //         });
    ...
    //             boxShadow: "0 0 2px red"
    //         });
    //         $('#userCue').html("<font color='red'><b>请输入正确格式邮箱</b></font>");
    //         return false;
    //     }
    //     //发送验证码
    //     var email = $("#email").val();
    ...
    $('#reg_button').click(function(){
        ...
        regusersn=/^\d{13}$/;
        // console.log(regusersn.test($('#user-sn').val()) == false);|| regusersn.test($('#user-sn').val()) == false
        if ($('#user-sn').val().length!=13 ) {
            $('#user-sn').focus().css({
                border: "1px solid red",
                boxShadow: "0 0 2px red"
            });
            $('#userCue').html("<font color='red'><b>请输入13位数字学号</b></font>");
            return false;
        }else {
            $('#user-sn').css({
                border: "1px solid #D7D7D7",
