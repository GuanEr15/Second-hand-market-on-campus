...
public interface GoodsDao extends JpaRepository<Goods, Long>,JpaSpecificationExecutor<Goods> {
	 ...
	 * @return
	 */
	@Query(value="SELECT * from guaner_goods where goods_category_id IN :cids and `status` = 1 ORDER BY create_time desc,flag desc,recommend desc limit :offset,:pageSize",nativeQuery=true)
	 ...
	 * @return
	 */
	@Query(value="SELECT count(*) from guaner_goods where goods_category_id IN :cids and `status` = 1 ",nativeQuery=true)
	 ...
	 * @return
	 */
	@Query(value="select * from guaner_goods where name like %:name%",nativeQuery=true)
