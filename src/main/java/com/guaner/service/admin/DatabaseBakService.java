package com.guaner.service.admin;
...
import com.guaner.dao.admin.DatabaseBakDao;
import com.guaner.entity.admin.DatabaseBak;
import com.guaner.util.StringUtil;
...
public class DatabaseBakService {
	...
	@Value("${guaner.database.backup.username}")
	private String dbUsername;

	@Value("${guaner.database.backup.password}")
	private String dbPwd;

	@Value("${guaner.database.backup.database.name}")
	...
	public void restore(Long id) {
		...
		if (databaseBak != null) {
			try {
				...
				;
				if (!file.exists()) {
					cmd = "mysql -u" + dbUsername + " -p" + dbPwd + " " + dbName + " < " + databaseBak.getFilepath()
							+ databaseBak.getFilename();
				}
